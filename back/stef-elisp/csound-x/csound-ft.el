...
(defun csft-refresh-display (ft-line)
  (let ((inhibit-read-only t))
    ...
    (condition-case nil
	(progn
	  (setq csft-plotting-area (csft-plot-ftable ft-line))
...
(defun csft-plot-ftable (ft-line)
  ...
  (let ((in-score (not (string-match "ftgen" ft-line)))
    ...
    (with-temp-file (cscsd-temp-csd-file)
      (insert "<CsoundSynthesizer>\n<CsOptions>\n--output=null\n</CsOptions>\n<CsInstruments>\n"
	      ...
	      "printk 0, kar\ngkindex = gkindex+" 
	      (number-to-string (/ (string-to-number ft-size) (float csft-width))) "\n"
	      "endin\n</CsInstruments>\n<CsScore>\n" 
	      (if in-score (csft-unnormalized ft-line) "")
	      "i1 0 " (number-to-string csft-width) " \ne\n</CsScore>\n</CsoundSynthesizer>"))
...
(defun csft-unnormalized (ft-line)
  (let ((genum (csft-ftable-gen ft-line)))
    (if (> (string-to-number genum) 0)
	(replace-match (format "-%s" genum) nil nil ft-line 3)
      ft-line)))
